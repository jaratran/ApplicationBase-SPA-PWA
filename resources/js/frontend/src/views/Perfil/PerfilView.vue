                            ...
                            <form @submit.prevent="updateAvatar">
                                <div class="form-group text-start px-2">
                                    ...
                                    <input type="file" id="avatarFile" ref="fileInput" accept="image/*"
                                        class="form-control" @change="submitAvatar" />
                                </div>
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-bordered table-striped table-sm">
                                <tbody>
                                    ...
                                    <tr>
                                        <th>Región</th>
                                        <td>{{ user.comuna?.region?.nombre }}</td>
                                    </tr>
                                </tbody>
                            </table>
...
<script setup>
    ...
    onMounted(async () => {
        // 1️⃣ Si el store YA tiene al usuario → usarlo directo
        if (auth.user) {
            user.value = auth.user
            return
        }

        // 2️⃣ Si no hay usuario → cargarlo desde la API
        const perfil = await auth.fetchPerfil()
        if (perfil) user.value = perfil
    })

    const avatarMedium = computed(() => {
        if (!user.value) return '/uploads/avatar/default_medium.jpg'
        return `/uploads/avatar/${user.value.avatar}_medium.jpg`
    })
    ...
    const submitAvatar = async () => {
        ...
        await axios.post('/api/perfil/avatar', form)
        user.value = await auth.fetchPerfil()
    }
